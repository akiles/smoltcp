...
impl<'a> InterfaceInner<'a> {
                   ...
                   Result<Option<IpPacket<'frame>>>
    {
        ...
        if !self.is_unicast(ipv4_repr.src_addr) {
            // Discard packets with non-unicast source addresses.
            net_debug!("non-unicast source address");
            return Err(Error::Malformed)
        }
        ...
        if !self.has_ip_addr(ipv4_repr.dst_addr) &&
           ...
           !self.is_broadcast(ipv4_repr.dst_addr) {
    ...
    /// Checks if an address is any broadcast (global or subnet)
    fn is_broadcast(&self, address: Ipv4Address) -> bool {
        address.is_broadcast() || self.is_subnet_broadcast(address)
    }

    /// Checks if an address is unicast, taking into account subnet broadcast addresses
    fn is_unicast(&self, address: Ipv4Address) -> bool {
                   ...
                   Option<IpPacket<'frame>>
    {
        if !self.is_unicast(ipv4_repr.src_addr) {
            ...
            None
        } else if self.is_unicast(ipv4_repr.dst_addr) {
        ...
        } else if self.is_broadcast(ipv4_repr.dst_addr) {
