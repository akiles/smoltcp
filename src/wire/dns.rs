...
impl<T: AsRef<[u8]>> Packet<T> {
    ...
    /// Returns the unused part of `bytes`.
    pub fn parse_name<'a>(
        &'a self,
        bytes: &'a [u8],
        ...
        let (rest, mut maybe_ptr) = parse_name_part(bytes, |label| f(label))?;
        while let Some(ptr) = maybe_ptr {
            ...
            let bytes = packet.get(ptr..).ok_or(Error::Malformed)?;
            packet = &packet[..ptr];

            let (_, next_ptr) = parse_name_part(bytes, |label| f(label))?;
            maybe_ptr = next_ptr;
        }
        Ok(rest)
    }
...
mod test {
    ...
    fn test_parse_name() {
        ...
        let name_vec = |bytes| {
            let mut v = Vec::new();
            packet.parse_name(bytes, |label| v.push(label)).map(|_| v)
        };
    ...
    fn test_parse_response() {
        ...
        assert_eq!(p.answers[0].data, RecordData::A(Ipv4Address::new(0xac, 0xd9, 0xa8, 0xae)));
    }
    ...
    fn test_parse_response_multiple_a() {
        ...
        assert_eq!(p.answers[0].data, RecordData::A(Ipv4Address::new(0x0d, 0xe0, 0x77, 0x35)));

        assert_eq!(p.answers[1].name, &[0xc0, 0x0c]);
        assert_eq!(p.answers[1].ttl, 9);
        assert_eq!(p.answers[1].data, RecordData::A(Ipv4Address::new(0x0d, 0xe0, 0x77, 0x28)));

        assert_eq!(p.answers[2].name, &[0xc0, 0x0c]);
        assert_eq!(p.answers[2].ttl, 9);
        assert_eq!(p.answers[2].data, RecordData::A(Ipv4Address::new(0x0d, 0xe0, 0x77, 0x43)));

        assert_eq!(p.answers[3].name, &[0xc0, 0x0c]);
        assert_eq!(p.answers[3].ttl, 9);
        assert_eq!(p.answers[3].data, RecordData::A(Ipv4Address::new(0x0d, 0xe0, 0x77, 0x62)));
    }
    ...
    fn test_parse_response_cname() {
        ...
        assert_eq!(p.answers[1].data, RecordData::A(Ipv4Address::new(0x1f, 0x0d, 0x53, 0x24)));
    }
